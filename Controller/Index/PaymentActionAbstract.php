...
use SwedbankPay\Payments\Api\Data\PaymentActionInterface;
...
{
    const EVENT_NAME_PREFIX = 'swedbank_pay_payments_';
     ...
     * @return JsonResult
     */
    protected function setResponse($key, $message = '', $httpCode = 200)
    {
        $result = $this->resultJsonFactory->create();
        $result->setData(array_merge($this->getEventArgs(), [$key => $message]));
        $result->setHttpResponseCode($httpCode);

        return $result;
    }

    protected function setRedirect($url, $code = 301)
    {
        /** @var \Magento\Framework\App\Response\HttpInterface $response */
        $response = $this->getResponse();
        $response->setRedirect($url, $code)->sendResponse();
    }
